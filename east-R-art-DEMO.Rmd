...
This is a demo version of the document with blank spaces to fill in code together.  The completed version can be see here: https://adamspannbauer.github.io/east-R-art/
...
ggplot() +
  geom_point(data = ______, aes(x = carat, y = price)) +
  geom_point(data = ______, aes(x = carat, y = price), ______) +
  theme_void()
```
...
ggplot() +
  geom_polygon(data = ______, aes(x = x, y = y)) +
  geom_polygon(data = ______, aes(x = x, y = y)) +
  theme_void()
```
...
canvas <- ggplot()
# TODO: make a for loop that adds an egg to the canvas
...
```{r}
# Make eggs with new function to rotate eggs
pts1 <- rotated_egg_pts(______)
pts2 <- rotated_egg_pts(______)
...
eggs <- list()
for (i in 1:n_xs) {
  # get value of x for this iteration
  # make rotated egg at location x

  # save to our egg carton
  eggs[[i]] <- egg
}
```
...
for (i in 1:n_xs) {
  for (j in 1:n_ys) {
    ...
    egg <- rotated_egg_pts(xs[i], ys[j], rotate_degrees = -90)
...
for (i in 1:n_xs) {
  for (j in 1:n_ys) {
    ...
    # Pick a random color from the palette and save
    color <- _______
    egg$color <- color
...
for (i in 1:n_xs) {
  for (j in 1:n_ys) {
    ...
    # Randomize rotation using a normal distribution centered at -90
    rotate <- _____
